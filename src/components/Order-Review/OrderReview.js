  ...
  return (
    <div className="bg-slate-100 shadow-2xl px-4 py-10">
      {cart.length <= 0 ? (
        ...
        <h1 className="text-cyan-700 text-3xl font-bold uppercase">
          Order review
        </h1>
      )}
      <div>
      ...
      <div className="w-full flex justify-evenly mt-10">
        {cart.length > 0 && (
          <Button btn_class="bg-cyan-700">review order</Button>
        )}
        {cart.length < 1 || (
          <Button onClick={removeAllItem} btn_class="bg-red-700">
            remove all
          </Button>
        )}
      </div>
